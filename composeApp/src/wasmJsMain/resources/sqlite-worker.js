...
// Wird: the durable database constructor, resolved once at init. Never fall back to a
// non-persistent VFS silently - a local-first app losing writes is worse than an error.
let DurableDb = null;
let durableVfsError = 'SQLite worker not initialised.';
...
function openRequest(id, requestData) {
    try {
        ...
        if (!DurableDb) {
            postMessage({'id': id, error: durableVfsError});
            return;
        }
        const newDatabase = new DurableDb(requestData.fileName);
...
sqlite3InitModule().then(async (instance) => {
    ...
    // Prefer the SAH Pool VFS: it calls createSyncAccessHandle directly in this worker,
    // so it needs no nested async-proxy worker - which some embedded browsers block.
    if (typeof sqlite3.installOpfsSAHPoolVfs === 'function') {
        try {
            const pool = await sqlite3.installOpfsSAHPoolVfs({name: 'wird-opfs-pool'});
            DurableDb = pool.OpfsSAHPoolDb;
            durableVfsError = null;
        } catch (error) {
            durableVfsError = 'OPFS SAH pool unavailable: ' + error.message;
        }
    }

    // Fall back to the async-proxy OPFS VFS only if it actually registered.
    if (!DurableDb && sqlite3.capi.sqlite3_vfs_find('opfs') !== 0) {
        DurableDb = sqlite3.oo1.OpfsDb;
        durableVfsError = null;
    }

    if (!DurableDb) {
        durableVfsError = (durableVfsError || 'No OPFS VFS available.') +
            ' Refusing to open a non-persistent database.';
    }
